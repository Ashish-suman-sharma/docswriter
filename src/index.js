...
try {
  ...
  if (fs.existsSync(dotenvPath)) {
    ...
    if (fs.existsSync(parentDotenvPath)) {
      console.log(`Loading environment variables from ${parentDotenvPath}`);
...
// No need to check for API key since we have it hardcoded
console.log('DocsWriter is ready to use with built-in API key.');
...
async function main() {
  try {
    ...
    if (argv.apiOnly) {
      ...
      const hasApiFiles = Object.keys(projectAnalysis.structure).some(key => 
        key.includes('routes') || 
